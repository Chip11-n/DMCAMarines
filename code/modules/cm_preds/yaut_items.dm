...
/obj/item/clothing/gloves/yautja/proc/translate()
	...
	msg = oldreplacetext(msg, "�", "@")
	msg = oldreplacetext(msg, "�", "3")
	msg = oldreplacetext(msg, "�", "1")
	msg = oldreplacetext(msg, "�", "0")
	msg = oldreplacetext(msg, "�", "7")
	msg = oldreplacetext(msg, "�", "9")
	msg = oldreplacetext(msg, "�", "5")
	msg = oldreplacetext(msg, "�", "3")
	msg = oldreplacetext(msg, "�", "*")
	...
	pickup(mob/living/user as mob)
		if(!isYautja(user))
			...
			if(do_after(user,30, TRUE, 5, BUSY_ICON_HOSTILE))
				...
				user.KnockDown(1)
				user.visible_message("<span class='warning'>It wasn't a good idea to take a long spear in one hand and try to walk like that. You lost your balance and fell.")
